...
def text_to_ids(source_text, target_text, source_vocab_to_int, target_vocab_to_int):
    ...
    for word in source_text.split('\n'):
        source_id_text.append([source_vocab_to_int[t] for t in word.split()])
    
    # Target 
    target_id_text = []
    for word in target_text.split('\n'):
        eos_word = word.split() + ['<EOS>']
        target_id_text.append([target_vocab_to_int[text] for text in eos_word])
...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob):
    ...
    output, lstm_state = tf.nn.dynamic_rnn(cell, rnn_inputs, dtype=tf.float32)
...
def decoding_layer_train(encoder_state, dec_cell, dec_embed_input, sequence_length, decoding_scope,
    ...
    """
    # TODO: Implement Function
    train = tf.contrib.seq2seq.simple_decoder_fn_train(encoder_state)
...
def decoding_layer_infer(encoder_state, dec_cell, dec_embeddings, start_of_sequence_id, end_of_sequence_id,
    ...
    """
    # TODO: Implement Function
    decode_inference = tf.contrib.seq2seq.simple_decoder_fn_inference(
        output_fn,
        encoder_state,
        dec_embeddings,
...
def seq2seq_model(input_data, target_data, keep_prob, batch_size, sequence_length, source_vocab_size, target_vocab_size,
    ...
    # 使用 encoding_layer 编码输入
    embed_sequence = tf.contrib.layers.embed_sequence(input_data, source_vocab_size, enc_embedding_size)
    encode = encoding_layer(embed_sequence, rnn_size, num_layers, keep_prob)
    
    # 使用 process_decoding_input 函数处理目标数据
    decoded_process = process_decoding_input(target_data, target_vocab_to_int, batch_size)
    embedding = tf.Variable(tf.random_uniform([target_vocab_size, dec_embedding_size]))  
    decoded_input = tf.nn.embedding_lookup(embedding, decoded_process)
    
    #使用 decoding_layer 函数解码编码的输入数据
...
def sentence_to_seq(sentence, vocab_to_int):
    ...
    for x in word:
        if(x in vocab_to_int):
            wordList.append(vocab_to_int[x])
        else:
